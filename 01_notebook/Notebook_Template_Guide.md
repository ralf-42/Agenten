...
**Wichtig:**
- `#@title Text { display-mode: "form" }` — collapsed by default; **immer mit Titel-Text**, nie leer
- ❌ Bare `#@title` (ohne Text) ist nicht zulässig und wird aus Notebooks entfernt
...
#### 2.3.1 Kapitel-Nummerierung - Ebene 1
...
### 3.4 Callout-Boxen (Merkregel, Fazit, Warnung)

Wiederkehrende Muster aus M01–M10 für didaktische Hervorhebungen:

```markdown
> 💡 **Merkregel:** Ein Agent ohne Tools ist ein LLM mit extra Latenz.

> 🎯 **FAZIT:** RAG schlägt Fine-Tuning, wenn die Wissensbasis häufig wechselt.

> ⚠️ **Achtung:** `recursion_limit` verhindert Endlosschleifen — Standard ist 25.

> 📝 **Hinweis:** `state_modifier` ist seit LangGraph 1.0 deprecated — `prompt` verwenden.
```
...
| Callout | Inhalt |
|---------|--------|
| `💡 Merkregel` | Kompakte Faustregeln, die man sich merken soll |
| `🎯 FAZIT` | Zusammenfassung nach Vergleich oder Hands-On |
| `⚠️ Achtung` | Häufige Fehler oder Einschränkungen |
| `📝 Hinweis` | Ergänzende Information, kein Kerninhalt |

### 3.5 Diagramme mit mermaid()

```python
from genai_lib.utilities import mermaid

#@markdown   <p><font size="4" color="green">Prozessdiagramm</font></p>
# Flowchart
diagram = '''
%%{init: {'theme':'light'}}%%
graph TD
    ...
    D --> E
''')
mermaid(diagram, width=580)


#@markdown   <p><font size="4" color="green">Sequenzdiagramm</font></p>
...
#@markdown   <p><font size="4" color="green">State Diagram</font></p>
...
### 4.6 Code-Organisations-Divider

In längeren Code-Zellen strukturieren visuelle Divider die Abschnitte — lesbar auch ohne Notebook-Renderer:
...
Zeichensatz: `─` (U+2500), erzeugt mit `─` oder Copy-Paste. Divider enden ca. bei Spalte 80.

### 4.7 LangSmith run_cfg Pattern

Jeder `agent.invoke()`- oder `chain.invoke()`-Aufruf bekommt einen run_cfg-Block für strukturiertes Tracing:

```python
run_cfg = {
    "run_name": "M12_RAG_Retrieval",
    "tags": ["rag", "retrieval", "m12"],
    "metadata": {"notebook": "M12", "version": "1.0"}
}

result = agent.invoke(
    {"messages": [("human", user_input)]},
    config={**session, **run_cfg}
)
```

**Namenskonvention:** `M##_[Thema]_[Schritt]` — erlaubt Filterung in LangSmith nach Modul.
...
### 5.5 Textliche Qualität

Textzellen in Notebooks folgen denselben Qualitätsregeln wie die Kursdokumentation.

**Regel 1 — Aufzählungen sparsam einsetzen**

Mehr als 50% Bullets in einem Abschnitt zeigt: der Inhalt wurde nicht formuliert, sondern aufgelistet. Stichpunkte eignen sich für echte Enumeration (Tools, Parameter, Schritte). Erklärungen, Begründungen und Zusammenhänge gehören in Fließtext.

**Regel 2 — Selbstbeschreibung vermeiden**

Nicht: *"Dieser Abschnitt zeigt, wie..."* — direkt mit dem Konzept beginnen. Das Notebook beschreibt sich nicht selbst, es erklärt eine Sache.

```markdown
# ❌ Selbstbeschreibend
Dieser Abschnitt erklärt, warum Checkpointing wichtig ist.

# ✅ Direkt
Checkpointing speichert den Graphzustand nach jedem Schritt — so kann ein Agent bei Unterbrechung dort weitermachen, wo er aufgehört hat.
```

**Regel 3 — Grenzen und Probleme benennen**

Mindestens ein Satz pro Konzept benennt, wann es nicht funktioniert oder was in der Praxis schiefgeht. Ohne diese Einordnung fehlt der didaktische Rahmen.

```markdown
> ⚠️ **Einschränkung:** MemorySaver speichert nur im RAM — bei Neustart des Kernels geht der Zustand verloren.
```

**Regel 4 — Erfahrungsbasierte Perspektive**

Praktische Einordnungen erhöhen die Relevanz. Formulierungen wie *"In der Praxis zeigt sich..."* oder *"Häufig wird unterschätzt..."* machen den Unterschied zwischen Referenz und Lernmaterial.

**Regel 5 — Keine Personalpronomina**

Notebooks verwenden unpersönliche Konstruktionen statt direkter Ansprache.

| ❌ Mit Ansprache | ✅ Unpersönlich |
|---|---|
| „Sie können den Agenten testen" | „Den Agenten testen" |
| „Versuchen Sie, den State..." | „Den State lässt sich..." |
| „Ergänzen Sie die Funktion" | „Die Funktion wird um... ergänzt" |

**Regel 6 — Kurzcheck vor dem Abschluss einer Textzelle**

Vor dem Speichern einer Markdown-Zelle drei Fragen prüfen:
1. Beginnt die Zelle mit dem Inhalt statt mit ihrer Selbstbeschreibung?
2. Gibt es mindestens einen Satz zu Einschränkungen oder typischen Fehlern?
3. Überwiegt Fließtext gegenüber Aufzählungspunkten?

### 5.6 „Was passiert hier?" — Erklär-Pattern nach Code

Nach komplexen Code-Zellen folgt eine Markdown-Zelle mit nummerierten Schritten, die den Ablauf Zeile für Zeile erklärt. Muster aus 6/10 Notebooks (M01–M10):

```markdown
**Was passiert hier?**

1. `StateGraph(AgentState)` — erzeugt einen neuen Graphen mit dem definierten State-Schema
2. `add_node("supervisor", supervisor_node)` — registriert den Supervisor als Knoten
3. `add_conditional_edges(...)` — legt fest, welcher Knoten nach dem Supervisor aktiv wird
4. `compile(checkpointer=memory)` — schließt den Graphen ab und aktiviert Checkpointing
```

**Wann verwenden:**
- Nach Zellen mit mehr als ~15 Zeilen Code
- Nach neuem LangGraph-Pattern (StateGraph, add_node, compile)
- Wenn mehrere Konzepte in einer Zelle zusammenkommen

**Wann weglassen:**
- Bei einfachen, selbsterklärenden Zellen (einzelner API-Aufruf, bekanntes Pattern)
- Wenn der Code bereits inline kommentiert ist
...
> **📖 Vollständige Dokumentation:** [Mermaid_Best_Practices.md](./Mermaid_Best_Practices.md)
> **Checkliste, häufige Fehler, Validierung und Code-Beispiele**
...
### 7.3 Schwierigkeitsstufen

Aufgaben werden in drei Stufen eingeteilt — konsistentes Muster aus M01–M10 (9/10 Notebooks):

```markdown
**🟢 Grundlagen** — Direktes Anwenden des Gelernten, Codezeilen ergänzen oder Parameter ändern.

**🟡 Aufbau** — Eigene Komponenten hinzufügen, bestehenden Graphen erweitern.

**🔴 Vertiefung** — Neues Szenario entwerfen, Architekturentscheidungen treffen.
```

**Verwendung:** Jede Aufgaben-Section benennt die Stufe explizit, sodass Lernende einschätzen können, welche Vorkenntnisse nötig sind.

### 7.4 assert-Pattern für Selbstchecks

Nach Hands-On Abschnitten validieren `assert`-Zellen das Ergebnis — gibt sofortiges Feedback ohne manuelles Prüfen:

```python
# Selbstcheck
assert result is not None, "❌ Kein Ergebnis — agent.invoke() prüfen"
assert len(result["messages"]) > 1, "❌ Nur eine Message — Tool-Aufruf fehlgeschlagen?"
assert "Antwort" in result["messages"][-1].content, "❌ Erwarteter Begriff fehlt"

print("✅ Alle Checks bestanden")
```

**Regeln:**
- Assert-Meldungen beginnen mit `❌` und nennen den wahrscheinlichen Grund
- Erfolg wird mit `print("✅ Alle Checks bestanden")` bestätigt
- Nur für klar prüfbare Eigenschaften — nicht für inhaltliche Qualität
...
- [ ] Schwierigkeitsstufen (🟢/🟡/🔴) für Aufgaben-Section vergeben
- [ ] assert-Pattern für Selbstchecks nach Hands-On Abschnitten

### 8.3a Textliche Qualität (Kurzcheck)
- [ ] Textzellen beginnen direkt mit dem Inhalt — keine Selbstbeschreibung ("Dieser Abschnitt zeigt...")
- [ ] Mindestens ein Satz zu Einschränkungen oder typischen Fehlern pro Konzept
- [ ] Fließtext überwiegt gegenüber Aufzählungspunkten (< 50% Bullets)
- [ ] Keine Personalpronomina ("Sie können..." → unpersönliche Konstruktion)
- [ ] Erfahrungsbasierte Formulierungen vorhanden ("In der Praxis...", "Häufig wird...")
...
**Changelog v2.1:**
- ✅ **Sektion 5.5 Textliche Qualität** — 6 Regeln aus Dokument_Standard (Aufzählungen, Selbstbeschreibung, Grenzen, Erfahrungsperspektive, Personalpronomina, Kurzcheck)
- ✅ **Sektion 3.4 Callout-Boxen** — `💡 Merkregel`, `🎯 FAZIT`, `⚠️ Achtung`, `📝 Hinweis` dokumentiert
- ✅ **Sektion 4.6 Code-Divider** — `─── Abschnitt ───` Pattern für längere Code-Zellen
- ✅ **Sektion 4.7 run_cfg Pattern** — LangSmith Tracing-Standard für alle invoke()-Aufrufe
- ✅ **Sektion 7.3 Schwierigkeitsstufen** — 🟢/🟡/🔴 Grundlagen/Aufbau/Vertiefung
- ✅ **Sektion 7.4 assert-Pattern** — Selbstchecks mit ❌/✅ Feedback
- ✅ **Checkliste 8.3/8.3a** — Didaktik und Textqualität erweitert
- ✅ **Sektion 5.6 „Was passiert hier?"** — Erklär-Pattern mit nummerierten Schritten nach komplexen Code-Zellen
- ✅ **Sektion 7.1** — Personalpronomina entfernt ("Sie können..." → unpersönlich)
