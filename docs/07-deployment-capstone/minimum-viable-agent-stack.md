...
Ein Agent ist nicht ein Chatbot mit mehr Prompts. Ein Chatbot braucht Inferenz und vielleicht RAG. Ein Agent benötigt Zustandsverwaltung über mehrstufige Ausführung, Werkzeugzugriff der von Protokollen gesteuert wird, Speicher der über Sitzungen hinweg erhalten bleibt, autonome Schlussschleifen und Leitplanken, die das Verhalten in Echtzeit einschränken. Das sind grundlegend andere Infrastrukturprobleme.
...
| Dokument | Frage |
|---|---|
| [Produktionsreife Anwendung]({{ '/07-deployment-capstone/aus-entwicklung-ins-deployment.html' | relative_url }}) | Wie wird der Übergang vom Jupyter Notebook zur produktionsreifen Anwendung technisch umgesetzt? |
| [Vom Modell zur Anwendung]({{ '/07-deployment-capstone/vom-modell-zum-produkt-langchain-oekosystem.html' | relative_url }}) | Welche LangChain-Komponenten sind für den Produktionseinsatz relevant? |
| [Agenten-Architekturen]({{ '/04-implementierung-frameworks-praxis/agent-architekturen.html' | relative_url }}) | Welche Architekturpattern gibt es für Agenten auf konzeptioneller Ebene? |
