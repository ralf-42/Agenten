...
| Dokument | Frage |
|---|---|
| [Wie bleiben Sitzungen und Zustände erhalten?]({{ '/memory-hitl/checkpointing-persistenz.html' | relative_url }}) | Wie wird State über Sitzungen hinweg gespeichert und wiederaufgenommen? |
| [Wie erinnern sich Agenten über mehrere Schritte und Sitzungen hinweg?]({{ '/memory-hitl/memory-systeme.html' | relative_url }}) | Wie wird aus dem laufenden State ein längerfristiges Gedächtnis? |
| [Multi-Agent-Systeme]({{ '/multi-agent/multi-agent-systeme.html' | relative_url }}) | Wie wird State zwischen mehreren Rollen oder Knoten eines Systems übergeben? |
