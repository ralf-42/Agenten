...
| Dokument | Frage |
|---|---|
| [Wie bleiben Sitzungen und Zustände erhalten?]({{ '/sessions-memory-hitl/checkpointing-persistenz.html' | relative_url }}) | Wie wird der State technisch gespeichert und wiederaufgenommen? |
| [State Management]({{ '/orchestrierung-state-langgraph/state-management.html' | relative_url }}) | Wie sind Nachrichten, Variablen und andere Zustandsdaten im Graph organisiert? |
| [Human-in-the-Loop]({{ '/sessions-memory-hitl/human-in-the-loop.html' | relative_url }}) | Wie wirkt sich gespeicherter Kontext auf Unterbrechung und Freigabe aus? |
